...
def student(client):
    ...
    r = client.post(
        "/api/v1/auth/login",
        json={"email": "reward@ecoloop.app", "password": "reward-pass-123"},
    )
...
def admin_auth(client):
    with SessionLocal() as db:
        if db.query(User).filter(User.email == "admin@ecoloop.app").first() is None:
            db.add(
                User(
                    id="u-admin",
                    email="admin@ecoloop.app",
...
def test_cash_redemption_lands_pending_in_admin_queue(client, student, admin_auth):
    ...
    assert item["student"]["email"] == "reward@ecoloop.app"
...
def test_stock_is_atomic_and_blocks_sold_out(client, student, admin_auth):
    ...
    for i in range(2):
        r = client.post(
            "/api/v1/auth/login",
            json={"email": f"stock{i}@ecoloop.app", "password": "stock-pass-123"},
        )
...
def _admin_headers(client) -> dict:
    ...
    r = client.post(
        "/api/v1/auth/login",
        json={"email": "race-admin@ecoloop.app", "password": "admin-pass-123"},
    )
    assert r.status_code == 200, r.text
    return {"Authorization": f"Bearer {r.json()['token']}"}
...
def test_concurrent_challenge_completion_keeps_base_points(
    ...
    with SessionLocal() as winner:
        wuser = winner.execute(
            select(User).where(User.email == "demo@ecoloop.app")
        ).scalar_one()
