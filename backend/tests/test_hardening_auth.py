...
def test_register_rate_limited(client, monkeypatch):
    ...
    codes = []
    for i in range(12):
        p = dict(payload, email=f"ratelimit{i}@ecoloop.app")
...
def test_forgot_password_never_enumerates(client, monkeypatch):
    ...
    unknown = client.post(
        "/api/v1/auth/forgot-password", json={"email": "nobody@ecoloop.app"}
    )
...
def test_registration_sends_verification_and_verify_email_works(
    ...
    with SessionLocal() as db:
        user = db.query(User).filter(User.email == "verify@ecoloop.app").first()
