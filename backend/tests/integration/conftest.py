...
def broker(tmp_path_factory):
    ...
    acl_file.write_text(
        f"user {BROKER_USER}\n"
        f"topic readwrite ecoloop/stations/#\n\n"
        f"# Least privilege: ST-001 may only touch its own topics.\n"
        f"user {STATION_USER}\n"
        f"topic read ecoloop/stations/st-001/command\n"
        f"topic read ecoloop/stations/st-001/capture_request\n"
        f"topic write ecoloop/stations/st-001/event\n"
        f"topic write ecoloop/stations/st-001/sensor\n"
        f"topic write ecoloop/stations/st-001/state\n"
        f"topic write ecoloop/stations/st-001/heartbeat\n"
    )
